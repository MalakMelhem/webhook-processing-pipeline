...
export async function createPipeline(pipelineName:string, webhookUrl:string, actions: string[]){
    const createdTime = new Date().toISOString(); 
    const query ='INSERT INTO pipelines (pipeline_name, webhook_url, created_time, actions) VALUES ($1,$2,$3,$4) RETURNING * ;';
    const values =[pipelineName,webhookUrl,createdTime, JSON.stringify(actions)];
...
export async function getSubscriber(pipelineId: number, subscriberUrl: string) {
    const query = 'SELECT * FROM subscribers WHERE pipeline_id = $1 and subscriber_url =$2 ';
    const result=await pool.query(query,[pipelineId, subscriberUrl]);
    return result.rows[0];
}
export async function updatePipleline(id: number, pipelineName: string, webhookUrl: string){
