...
import { getPipleline, createPipeline, updatePipleline, deletePipleline } from "./db/queries.js";
...
app.get("/pipelines/:pipelineName", async(req: Request,res: Response)=>{
  const { pipelineName }= req.params;
try{
 const pipeline= await getPipleline(pipelineName as string);
 if (!pipeline) {
